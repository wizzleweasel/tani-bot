...
description: "External service operations via composio_helper.py. Use when: (1) user asks about GitHub, (2) user asks about Notion, (3) user asks about Supabase, (4) user asks about Hacker News. Always run bash commands — never call tools directly."
---
...
Connected services: GitHub, Notion, Supabase, Hacker News
...
# Notion via the composio-run command

**Always execute these as bash commands. Never attempt to call a tool named composio or notion directly.**

## Search pages and databases

```bash
/mnt/data/openclaw/workspace/bin/composio-run exec NOTION_SEARCH_NOTION_PAGE '{"query": "meeting notes"}'
```

## Create a new page

```bash
/mnt/data/openclaw/workspace/bin/composio-run exec NOTION_CREATE_NOTION_PAGE '{"parent_id": "DATABASE_OR_PAGE_ID", "title": "New Page"}'
```

## Add content to a page

```bash
/mnt/data/openclaw/workspace/bin/composio-run exec NOTION_ADD_MULTIPLE_PAGE_CONTENT '{"page_id": "PAGE_ID", "content": "Content to add"}'
```

## Fetch a page

```bash
/mnt/data/openclaw/workspace/bin/composio-run exec NOTION_FETCH_PAGE '{"page_id": "PAGE_ID"}'
```

## Create a database

```bash
/mnt/data/openclaw/workspace/bin/composio-run exec NOTION_CREATE_DATABASE '{"parent_id": "PAGE_ID", "title": "My Database"}'
```

## Add a comment

```bash
/mnt/data/openclaw/workspace/bin/composio-run exec NOTION_CREATE_COMMENT '{"page_id": "PAGE_ID", "content": "A comment"}'
```

## List all users

```bash
/mnt/data/openclaw/workspace/bin/composio-run exec NOTION_LIST_ALL_USERS '{}'
```

## Rules

1. Always use bash to execute the composio-run command. Never call a tool directly.
2. Use NOTION_SEARCH_NOTION_PAGE first to find page/database IDs before other operations.
